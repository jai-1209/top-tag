# Tagger to train, supported options are 'hldnn', 'dnn', 'pfn'
tagger_type = 'pfn'
...
train_path =  "ATLAS_TopTagging_OpenData_1M.h5"
test_path =  "ATLAS_TopTagging_OpenData_1M.h5"

# take a subset of the data. Using the full set would require more memory.
...
# Find names of appropriate numpy arrays from h5py file objects
...
# Load data into a Python dictionary for passing into pre-processing functions
...
if tagger_type == 'hldnn':
    ...
    train_data = high_level(train_dict)
    test_data = high_level(test_dict)
    num_data_features = train_data.shape[-1]
else:
    # Data shapes: (n_jets, max_constits, 7)
    print("Processing constituent information")
    train_data = constituent(train_dict, max_constits)
    test_data = constituent(test_dict, max_constits)
    num_data_features = train_data.shape[-1]

# Load labels and training weights
train_labels = train["labels"][:n_train_jets]
train_weights = train["training_weights"][:n_train_jets]
...
# This block will configure the architecture you've specified:
# You may want to experiment with changing the hyperparameters here in order
# to see how the network performance changes.
...
if tagger_type == "efn":
    ...
    # a single dataset. We want each element of the dataset to have shape:
    ...
    (train_angular, valid_angular, train_pt, valid_pt, train_labels, valid_labels, train_weights, valid_weights) = train_test_split(
        train_angular,
        train_pt,
        train_labels,
        train_weights,
    ...
    train_sets = tuple(Dataset.from_tensor_slices(i).batch(batch_size) for i in train_list)
    ...
    valid_sets = tuple(Dataset.from_tensor_slices(i).batch(batch_size) for i in valid_list)
    ...
    test_sets = tuple(Dataset.from_tensor_slices(i).batch(batch_size) for i in test_list)
    test_data = Dataset.zip(test_sets[:2])
    test_dataset = Dataset.zip((test_data,) + test_sets[2:])

# For all other models, datasets can be built using the same process,
# these are handled together else: # Finally make a plot of the background rejection versus jet pT.
    ...
    else: # Finally make a plot of the background rejection versus jet pT.
        # Start by making a set of pT bins and empty vectors to accept B.R. values.
        pt_bins = np.linspace(350000, 3150000, 15) # Indent this block
        br_point5_array = np.zeros(len(pt_bins) - 1) # Indent this block
        br_point8_array = np.zeros(len(pt_bins) - 1) # Indent this block

        # Loop through pT bins # Indent this block
        for i in range(len(pt_bins) - 1): # Indent this block
            # Find the indices of the jets that lie in this pT bin # Indent this block
            condition = np.logical_and(jet_pt > pt_bins[i], jet_pt < pt_bins[i+1]) # Indent this block
            bin_indices = condition.nonzero()[0] # Indent this block

            # Now take predictions and labels which fall in this pT bin # Indent this block
            bin_predictions = predictions[bin_indices] # Indent this block
            bin_labels = test_labels[bin_indices] # Indent this block

            # Calculate the background rejection at fixed signal efficiency # Indent this block
            fpr, tpr, thresholds = metrics.roc_curve(bin_labels, bin_predictions) # Indent this block
            bin_point5_index = np.argmax(tpr > 0.5) # Indent this block
            br_point5_array[i] = 1 / fpr[bin_point5_index] # Indent this block
            bin_point8_index = np.argmax(tpr > 0.8) # Indent this block
            br_point8_array[i] = 1 / fpr[bin_point8_index] # Indent this block

        # Duplicate last entry in background rejection arrays # Indent this block
        br_point5_array = np.concatenate((br_point5_array, br_point5_array[-1:])) # Indent this block
        br_point8_array = np.concatenate((br_point8_array, br_point8_array[-1:])) # Indent this block

        # Make a plot using matplotlib's step function # Indent this block
        plot_bins = pt_bins / 1e6  # Set plot on TeV scale # Indent this block
        plt.step(plot_bins, br_point5_array, where="post", label=r"$\epsilon (sig) = 0.5$") # Indent this block
        plt.step(plot_bins, br_point8_array, where="post", label=r"$\epsilon (sig) = 0.8$") # Indent this block
        plt.ylabel("Background rejection") # Indent this block
        plt.xlabel("Jet pT (TeV)") # Indent this block
        plt.legend() # Indent this block
        plt.savefig(figure_dir / 'br_vs_pt.png', dpi=300) # Indent this block

        raise ValueError("Tagger type setting not recognized") # Indent this block




# Make train/valid split using sklearn train_test_split function
(train_data, valid_data, train_labels, valid_labels, train_weights, valid_weights) = train_test_split(
    train_data,
    train_labels,
    train_weights,
    test_size=valid_fraction
...
# Train tagger with keras fit function. Use validation_data argument to
...
# Run prediction on the testing set. Make cut on tagger output at 0.5
...
# Finally make a plot of the background rejection versus jet pT.
# Start by making a set of pT bins and empty vectors to accept B.R. values.
...
plt.step(plot_bins, br_point5_array, where="post", label=r"$\epsilon (sig) = 0.5$")
plt.step(plot_bins, br_point8_array, where="post", label=r"$\epsilon (sig) = 0.8$")
plt.ylabel("Background rejection")
